...
class Post_Date extends Tag {
	...
	public function get_title() {
		return __( 'Post Date', 'one-elements' );
	}
	...
	protected function _register_controls() {
		$this->add_control(
			...
			[
				...
				'options' => [
					'post_date_gmt' => __( 'Post Published', 'one-elements' ),
					'post_modified_gmt' => __( 'Post Modified', 'one-elements' ),
				],
	...
	public function render() {
		...
		if ( 'human' === $format ) {
			...
			$value = sprintf( __( '%s ago', 'one-elements' ), human_time_diff( strtotime( get_post()->{$date_type} ) ) );
		} else {
			switch ( $format ) {
				case 'default':
					$date_format = '';
