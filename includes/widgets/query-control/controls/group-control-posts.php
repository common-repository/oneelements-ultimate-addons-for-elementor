...
use OneElements\Includes\Widgets\QueryControl\Module;
...
class Group_Control_Posts extends Group_Control_Base {
	...
	public static function on_export_remove_setting_from_element( $element, $control_id ) {
		...
		foreach ( one_get_public_post_types() as $post_type => $label ) {
	...
	protected function prepare_fields( $fields ) {
		...
		$post_types_options['current_query'] = __( 'Current Query', 'one-elements' );
	...
	protected function build_query_args( $settings, $control_id_prefix ) {
		...
		if ( 'by_id' === $post_type ) {
			$post_types = one_get_public_post_types();
	...
	public function get_query_args( $control_id_prefix, $settings ) {
		...
		if ( 'current_query' === $post_type ) {
			 ...
			 */
			$current_query_vars = apply_filters( 'one_elements/query_control/get_query_args/current_query', $current_query_vars );
