<?php
namespace OneElements\Includes\Widgets\QueryControl\Controls;

use Elementor\Controls_Manager;
use Elementor\Group_Control_Base;
use OneElements\Includes\Utils;
use OneElements\Includes\Widgets\QueryControl\Module as Query_Module;
...
class Group_Control_Query extends Group_Control_Base {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type'] = [
			'label' => __( 'Source', 'one-elements' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'by_id' => __( 'Manual Selection', 'one-elements' ),
				'current_query' => __( 'Current Query', 'one-elements' ),
			],
		];
		...
		$fields['include'] = [
			'label' => __( 'Include By', 'one-elements' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'terms' => __( 'Term', 'one-elements' ),
				'authors' => __( 'Author', 'one-elements' ),
			],
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['exclude'] = [
			'label' => __( 'Exclude By', 'one-elements' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'current_post' => __( 'Current Post', 'one-elements' ),
				'manual_selection' => __( 'Manual Selection', 'one-elements' ),
				'terms' => __( 'Term', 'one-elements' ),
				'authors' => __( 'Author', 'one-elements' ),
			],
			'condition' => [
		...
		$fields['avoid_duplicates'] = [
			'label' => __( 'Avoid Duplicates', 'one-elements' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
			'description' => __( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'one-elements' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
			'condition' => [
				'post_type!' => [
		...
		$fields['offset'] = [
			...
			'description' => __( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'one-elements' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
		];

		$fields['select_date'] = [
			'label' => __( 'Date', 'one-elements' ),
			'type' => Controls_Manager::SELECT,
			'post_type' => '',
			'options' => [
				'anytime' => __( 'All', 'one-elements' ),
				'today' => __( 'Past Day', 'one-elements' ),
				'week' => __( 'Past Week', 'one-elements' ),
				'month'  => __( 'Past Month', 'one-elements' ),
				'quarter' => __( 'Past Quarter', 'one-elements' ),
				'year' => __( 'Past Year', 'one-elements' ),
				'exact' => __( 'Custom', 'one-elements' ),
			],
			'default' => 'anytime',
			'label_block' => false,
			'multiple' => false,
		...
		$fields['date_before'] = [
			...
			'filter_type' => 'date',
			'include_type' => true,
			'placeholder' => __( 'Choose', 'one-elements' ),
			'condition' => [
				'select_date' => 'exact',
				'post_type!' => [
					'by_id',
			...
			'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'one-elements' ),
		];

		$fields['date_after'] = [
			'label' => __( 'After', 'one-elements' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			...
			'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'one-elements' ),
		];
		...
		$fields['orderby'] = [
			...
			'default' => 'post_date',
			'options' => [
				'post_date' => __( 'Date', 'one-elements' ),
				'post_title' => __( 'Title', 'one-elements' ),
		...
		$fields['order'] = [
			...
			'default' => 'desc',
			'options' => [
				'asc' => __( 'ASC', 'one-elements' ),
				'desc' => __( 'DESC', 'one-elements' ),
			],
			'condition' => [
				'post_type!' => 'current_query',
			],
		...
		$fields['ignore_sticky_posts'] = [
			'label' => __( 'Ignore Sticky Posts', 'one-elements' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
			'condition' => [
				'post_type' => 'post',
			],
			'description' => __( 'Sticky-posts ordering is visible on frontend only', 'one-elements' ),
		];

		$fields['query_id'] = [
			'label' => __( 'Query ID', 'one-elements' ),
			'type' => Controls_Manager::TEXT,
			'default' => '',
			'description' => __( 'Give your Query a custom unique id to allow server side filtering', 'one-elements' ),
			'separator' => 'before',
		];
	...
	protected function prepare_fields( $fields ) {
		...
		$post_types = one_get_public_post_types( $post_type_args );
